...
> Mis à jour : Session 77 — Mars 2026
...
**Convention d'appending** : les hooks d'une nouvelle session sont ajoutés à la fin du fichier existant avec un commentaire `// ─── S76 ───`.
...
users (
  ...
  email           text,
  -- PAS de colonne full_name dans certains projets — vérifier avant usage
)
```
...
### Tâches avancées (S77)
```sql
task_dependencies (organization_id, task_id, depends_on_id, dependency_type)
--  dependency_type : 'blocks' | 'related'

task_recurrences (organization_id, task_id, frequency, interval_value, days_of_week, end_date)
--  frequency : 'daily' | 'weekly' | 'monthly' | 'custom'

task_time_tracking (organization_id, task_id, user_id, minutes_spent, logged_at, note)

-- Colonnes ajoutées sur tasks :
--   estimated_minutes integer
--   workload_score    integer (0-10)
```

---

## 5. Modules en production (S1 → S76)
