...
> Mis à jour : Session 71 — Gestion des Temps — Règles heures sup + export paie ✅ (08/03/2026)
...
34. **Gestion des Temps — Règles HS + Export paie DÉPLOYÉ (S71)** :
    - `useTemps.js` — 6 nouveaux hooks : `useRecalculateOvertime`, `useRecalculateOrgOvertime`, `useApproveOvertime`, `useRejectOvertime`, `useOvertimeAlerts`, `useExportOvertimePayroll`, `usePendingOvertimeSheets`
    - `calculateOvertimeBreakdown()` — calcul HS modes journalier/hebdo/both
    - `DEFAULT_OT_SETTINGS` + `OT_MODES` — constantes moteur
    - `OvertimeRulesEngine.jsx` — config seuils, taux, alertes, recalcul org
    - `OvertimeSummary.jsx` — récapitulatif 4 semaines avec barres empilées SVG
    - `OvertimeValidation.jsx` — workflow approbation HS pour managers
    - `OvertimeAlerts.jsx` — panneau alertes proactives (retard, HS pending, volume)
    - `OvertimePayrollExport.jsx` — export CSV/XLSX mensuel : récap + détail hebdo
    - `GestionTemps.jsx` — 5 onglets : Ma Feuille / Mon Équipe / Heures Sup. / Alertes / Administration
    - `s71_temps_heures_sup.sql` — 9 colonnes time_settings + 8 colonnes time_sheets + MV mensuelle
...
## Diagnostic modules — état post S71
