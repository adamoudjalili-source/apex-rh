...
// ✅ S9    : auto-assignation créateur, log, user→profile
// ✅ S128  : bouton "Depuis un template" + TaskTemplateSelector
// ============================================================
    ...
    async function loadOrgData() {
      ...
      if (!isAdmin && !isDirecteur) {
        ...
        const { data: dirDivs } = await supabase
          .from('divisions').select('id')
          .eq('direction_id', profile.direction_id).eq('is_active', true)
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (task) {
        ...
        if (form.title !== task.title)                  { logAction = 'title_changed';    logOld = task.title;     logNew = form.title }
        else if (form.priority !== task.priority)        { logAction = 'priority_changed'; logOld = task.priority;  logNew = form.priority }
        else if (form.due_date !== (task.due_date || '')) { logAction = 'due_date_changed'; logOld = task.due_date || 'non définie'; logNew = form.due_date || 'non définie' }
        else if (form.status   !== task.status)          { logAction = 'status_changed';   logOld = task.status;    logNew = form.status }
        await updateTask.mutateAsync({ taskId: task.id, updates: taskData, logAction, logOld, logNew })
      } else {
  ...
  return (
    <>
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        {/* Bouton "Depuis un template" — uniquement pour une nouvelle tâche */}
        {!task && (
          <div className="flex items-center gap-2 pb-1 border-b border-white/5">
            <button type="button" onClick={() => setShowTemplates(true)}
              className="flex items-center gap-2 px-3 py-1.5 text-xs text-indigo-400 hover:text-indigo-300 border border-indigo-500/30 hover:border-indigo-500/50 rounded-lg transition-colors bg-indigo-500/5">
              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />
              </svg>
              Depuis un template
            </button>
            <span className="text-xs text-gray-600">ou remplissez le formulaire ci-dessous</span>
          </div>
        )}
        ...
        <div>
          ...
          <input autoFocus type="text" value={form.title} onChange={e => set('title', e.target.value)} placeholder="Nom de la tâche..."
        ...
        <div>
          ...
          <textarea value={form.description} onChange={e => set('description', e.target.value)} placeholder="Décrivez la tâche..." rows={3}
        ...
        <div>
          ...
          <input type="number" min="0" step="0.5" value={form.estimated_hours} onChange={e => set('estimated_hours', e.target.value)} placeholder="ex: 4.5"
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <select value={form.division_id} onChange={e => set('division_id', e.target.value)}
              ...
              {divisions.map(d => <option key={d.id} value={d.id} className="bg-[#1a1a35] text-gray-200">{d.name}</option>)}
            </select>
          </div>
          <div>
            ...
            <select value={form.service_id} onChange={e => set('service_id', e.target.value)}
              ...
              {services.map(s => <option key={s.id} value={s.id} className="bg-[#1a1a35] text-gray-200">{s.name}</option>)}
            </select>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="max-h-36 overflow-y-auto space-y-1 border border-white/10 rounded-lg p-2 bg-white/3">
            {users.map(u => (
              <label key={u.id}
                className={`flex items-center gap-2 px-2 py-1.5 rounded-md cursor-pointer transition-colors ${assigneeIds.includes(u.id) ? 'bg-indigo-500/20' : 'hover:bg-white/5'}`}>
                <input type="checkbox" checked={assigneeIds.includes(u.id)} onChange={() => toggleAssignee(u.id)} className="accent-indigo-500" />
