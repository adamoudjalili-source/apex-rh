// ============================================================
// APEX RH — MonEspace.jsx  ·  Session 37
// Espace personnel complet :
//   - Profil multi-dimensionnel 5 axes (score IPR unique SUPPRIMÉ)
//   - Tâches actives
//   - Sparkline PULSE 30j
          ...
          <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            ...
            {pulseOn && (
              <div className="flex items-center gap-5 flex-shrink-0">
                {/* Profil qualitatif — PAS de chiffre IPR unique */}
                {!iprLoading && iprData?.dimensions && (
                  <div className="flex flex-col items-center gap-2">
                    {(() => {
                      const dims = iprData.dimensions
                      const weighted = Object.values(dims)
                        .filter(d => d.score !== null && d.score !== undefined)
                        .reduce((acc, d) => acc + d.score * (d.weight || 0.2), 0)
                      const tw = Object.values(dims)
                        .filter(d => d.score !== null && d.score !== undefined)
                        .reduce((acc, d) => acc + (d.weight || 0.2), 0)
                      const globalScore = tw > 0 ? Math.round(weighted / tw) : null
                      const ql = getQualitativeLabel(globalScore)
                      return (
                        <>
                          <div className="w-20 h-20 rounded-2xl flex items-center justify-center"
                            style={{ background: ql.bg, border: `2px solid ${ql.border}`, boxShadow: `0 0 24px ${ql.color}22` }}>
                            <span className="text-2xl font-black leading-none" style={{ color: ql.color, fontFamily:"'Syne',sans-serif" }}>
                              {ql.label.charAt(0)}
                            </span>
                          </div>
                          <div className="text-center">
                            <span className="text-xs font-bold" style={{ color: ql.color }}>{ql.label}</span>
                            {iprTrend !== null && iprTrend !== undefined && (
                              <div className="mt-0.5"><TrendBadge value={iprTrend} suffix="pts" size="sm" /></div>
                            )}
                          </div>
                        </>
                      )
              ...
              <SectionCard
                ...
                {perfTab === 0 && (
                  <div className="space-y-3">
                    {iprData?.dimensions ? (
                      <ProfilPerformance iprData={iprData} isManager={false} compact showRadar={false} />
                    ) : (
                      <div className="grid grid-cols-3 gap-3">
                        <MetricCard label="PULSE moy." value={sparkline30.length?Math.round(sparkline30.reduce((a,b)=>a+b,0)/sparkline30.length):null}
                          suffix="/100" color="#4F46E5" icon={<TrendingUp size={12}/>} size="sm" />
                        <MetricCard label="Jours actifs" value={iprData?.daysWithData??0}
                          color="#C9A227" icon={<Calendar size={12}/>} size="sm" />
                        <MetricCard label="Objectifs" value={iprData?.okrCount??0}
                          color="#8B5CF6" icon={<Target size={12}/>} size="sm" />
                      </div>
                    )}
                  </div>
                )}
                ...
                {perfTab === 1 && iprData?.dimensions && (
                  <ProfilPerformance iprData={iprData} isManager={false} compact={false} showRadar />
                )}
