...
// Session 70 — Congés & Absences — Moteur de règles complet
// Onglets : Mes Congés · Mon Équipe · Alertes · Administration
// ============================================================
...
function MesConges() {
  ...
  return (
    <div className="space-y-6">
      {showForm && <LeaveRequestForm onClose={() => setShowForm(false)} onSuccess={() => { setShowForm(false); refetch() }}/>}
      ...
      <div>
        <h2 className="text-xs font-semibold text-white/30 uppercase tracking-wider mb-3">Mes soldes — {yearFilter}</h2>
        <LeaveBalanceCard year={yearFilter}/>
      </div>
      <div>
        <h2 className="text-xs font-semibold text-white/30 uppercase tracking-wider mb-3">
          Mes demandes{requests.length > 0 && <span className="ml-2 text-white/40 normal-case font-normal">({filtered.length})</span>}
        </h2>
        {isLoading ? (
          <div className="flex items-center justify-center py-10"><div className="w-6 h-6 rounded-full border-2 border-indigo-500 border-t-transparent animate-spin"/></div>
        ) : filtered.length === 0 ? (
          <div className="rounded-2xl px-5 py-12 text-center" style={{ background:'rgba(255,255,255,0.02)', border:'1px dashed rgba(255,255,255,0.07)' }}>
            <CalendarOff size={32} className="mx-auto text-white/15 mb-3"/>
            <p className="text-white/30 text-sm">Aucune demande de congé</p>
            <button onClick={() => setShowForm(true)} className="mt-4 text-xs text-indigo-400 hover:underline">+ Créer une demande</button>
          </div>
        ) : (
          <div className="space-y-2">{filtered.map(r => <LeaveRequestCard key={r.id} request={r}/>)}</div>
        )}
      </div>
    </div>
  )
  ...
  return (
    <div className="space-y-5">
      <div className="flex gap-1 p-1 rounded-xl w-fit" style={{ background:'rgba(255,255,255,0.04)' }}>
        {[{ key:'pending', label:`À valider (${pending.length})` }, { key:'calendar', label:'Calendrier' }, { key:'all', label:'Toutes' }].map(t => (
          <button key={t.key} onClick={() => setView(t.key)}
            className={`px-4 py-1.5 rounded-lg text-xs font-medium transition-all ${view===t.key?'bg-indigo-600 text-white':'text-white/40 hover:text-white/60'}`}>
            {t.label}
          </button>
        ))}
      </div>
      ...
      {view === 'all' && (
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={() => exportLeaves.mutateAsync({ startDate:`${now.getFullYear()}-01-01`, endDate:`${now.getFullYear()}-12-31` })}
          ...
          <div className="space-y-2">{all.map(r => <LeaveRequestCard key={r.id} request={r} showUser/>)}</div>
        </div>
      )}
    </div>
  )
...
// ─── Onglet Alertes ───────────────────────────────────────────
function AlertesTab() {
  return <div className="space-y-6"><LeaveAlerts/></div>
}
...
function Administration() {
  ...
  const [adminView, setAdminView] = useState('general')
    ...
    <div className="space-y-5">
      <div className="flex gap-1 p-1 rounded-xl w-fit" style={{ background:'rgba(255,255,255,0.04)' }}>
        {[
          ...
          { key:'rules',    label:"Règles d'accrual" },
          { key:'holidays', label:'Jours fériés' },
          { key:'payroll',  label:'Export paie' },
        ].map(t => (
          <button key={t.key} onClick={() => setAdminView(t.key)}
            className={`px-4 py-1.5 rounded-lg text-xs font-medium transition-all ${adminView===t.key?'bg-violet-600 text-white':'text-white/40 hover:text-white/60'}`}>
            {t.label}
          </button>
        ))}
      </div>
      ...
      {adminView === 'general' && (
        <div className="space-y-6">
          <div>
            <h2 className="text-xs font-semibold text-white/30 uppercase tracking-wider mb-4">Paramètres organisation</h2>
            <div className="rounded-2xl border p-5 space-y-5" style={{ background:'rgba(255,255,255,0.025)', borderColor:'rgba(255,255,255,0.08)' }}>
              {isLoading ? (
                <div className="h-32 flex items-center justify-center"><div className="w-5 h-5 rounded-full border-2 border-indigo-500 border-t-transparent animate-spin"/></div>
              ) : (
                <>
                  <div className="grid grid-cols-2 gap-4">
                    {[
                      ...
                      { key:'low_balance_threshold',  label:'Seuil alerte solde (j)',  min:0, max:15  },
                      { key:'pending_alert_hours',    label:'Alerte attente (heures)', min:0, max:168 },
                    ].map(({ key, label, min, max }) => (
                      <div key={key}>
                        <label className="text-xs text-white/40 mb-1.5 block">{label}</label>
                        <input type="number" min={min} max={max} value={(current?.[key] ?? '')}
                    ...
                    <div>
                      ...
                      <input type="date" value={current?.carry_over_deadline || ''}
                  ...
                  <button onClick={handleSave} disabled={!form || updateSettings.isPending}
      ...
      {adminView === 'rules'    && <LeaveRulesEngine/>}
      {adminView === 'holidays' && <PublicHolidaysManager/>}
      {adminView === 'payroll'  && <LeavePayrollExport/>}
    </div>
  )
}
...
export default function GestionConges() {
  ...
  const tabs = [
    ...
    { key:'alertes',    label: criticalAlerts > 0 ? `Alertes (${criticalAlerts})` : 'Alertes', icon: criticalAlerts > 0 ? AlertTriangle : Bell, show: canValidate, badge: criticalAlerts > 0 },
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      <div className="flex-shrink-0 px-6 pt-6 pb-0">
        <div className="flex items-start justify-between mb-5">
          <div>
            ...
            <p className="text-xs text-white/30 mt-0.5">Moteur de règles · Jours fériés · Report N+1 · Export paie</p>
          </div>
        </div>
          ...
          {tabs.map(tab => {
            ...
            return (
              <button key={tab.key} onClick={() => setActiveTab(tab.key)}
                className={`relative flex items-center gap-1.5 px-4 py-2.5 text-sm font-medium border-b-2 transition-all ${active?'text-violet-400 border-violet-400':'text-white/30 border-transparent hover:text-white/55'}`}>
                <Icon size={14}/>
                {tab.label}
      ...
      <div className="flex-1 overflow-y-auto px-6 py-6">
        ...
        {activeTab === 'alertes'    && canValidate   && <AlertesTab/>}
