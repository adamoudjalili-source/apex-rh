...
// ─── S39 — PONDÉRATION TÂCHES ────────────────────────────────
/**
 * Calcule le score Delivery pondéré côté client
 * Prend en compte les critères de pondération de chaque tâche
 *
 * @param {Array} tasks — liste de tâches avec weight_* et status
 * @returns {number} score Delivery pondéré 0–100
 */
export function computeWeightedDelivery(tasks = []) {
...
/**
 * Retourne le label d'une valeur de pondération (1–5)
 */
export function getWeightLevelLabel(v) {
  const labels = { 1: 'Minimal', 2: 'Faible', 3: 'Moyen', 4: 'Élevé', 5: 'Critique' }
  return labels[v] || 'Moyen'
}

/**
 * Calcule le score de pondération global d'une tâche (0–100)
 */
export function getTaskWeightScore(task) {
  if (!task) return 20
